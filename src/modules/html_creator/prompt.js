...
export function buildPrompt({ lang = 'ru' }) {
  ...
  return `[System Note: Whenever the scene contains a visual object (screen, letter, note, document, UI, sign, chat, app interface, poster, card, map, HUD, display), render it as a rich live CSS-only HTML block. If the scene is pure dialogue with no visual object, you may skip. Wrap EXACTLY as raw HTML inside: <rs_art>YOUR_RAW_HTML_HERE</rs_art>. Output at most one <rs_art> block per message. No markdown fences and no HTML comment wrapper. Include continuous CSS animation via a <style> block. Use only CSS/HTML, no scripts. All generated classes must use rsa- or rs-art- prefixes. Use unique ids with prefix rsa_. Allowed interaction attributes: data-rs-action, data-rs-reply, data-rs-send, data-rs-flip, data-rs-face. ${visibleRule} Vibe hints: ${pick(VIBES, 3).join('; ')}. ${pick(DETAIL_BOOSTS, 1)[0]}]`;
}