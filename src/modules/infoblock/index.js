...
function renderCharacters(characters, lang) {
  ...
  return characters.map((c) => {
    ...
    return `<div class="${PFX}-char-entry"><span class="${PFX}-char-name">${esc(c.name)}</span><div class="${PFX}-char-details"><span>👕 ${mdInline(c.outfit || na)}</span><span>🧘 ${mdInline(c.position || na)}</span><span>❤️ ${mdInline(c.condition || na)}</span></div></div>`;
  }).join('');
}
...
export function render(data, { lang = 'ru', currentState = null } = {}) {
  ...
  return `<div class="${PFX}-container collapsed"><div class="${PFX}-header" data-${PFX}-toggle><span class="${PFX}-chevron">▼</span><span class="${PFX}-header-text">${getLabel('header', lang)}</span></div><div class="${PFX}-body"><div class="${PFX}-block time"><span class="${PFX}-label">${getLabel('dateTime', lang)}</span><span class="${PFX}-value">${dateTime}</span></div><div class="${PFX}-block location"><span class="${PFX}-label">${getLabel('location', lang)}</span><span class="${PFX}-value">${mdInline(state.location)}</span></div><div class="${PFX}-block global"><span class="${PFX}-label">${getLabel('globalConflict', lang)}</span><span class="${PFX}-value">${state.global_conflict ? mdInline(state.global_conflict) : '—'}</span></div><div class="${PFX}-block characters"><span class="${PFX}-label">${getLabel('characters', lang)}</span><div class="${PFX}-char-list">${renderCharacters(state.characters, lang)}</div></div><div class="${PFX}-block detail"><span class="${PFX}-label">${getLabel('keyDetail', lang)}</span><span class="${PFX}-value">${state.key_detail ? mdInline(state.key_detail) : '—'}</span></div><div class="${PFX}-block summary"><span class="${PFX}-label">${getLabel('plotSummary', lang)}</span><div class="${PFX}-value">${state.plot_summary ? mdBlock(state.plot_summary) : '—'}</div></div><div class="${PFX}-block future"><span class="${PFX}-label">${getLabel('futurePlot', lang)}</span><div class="${PFX}-value">${state.future_plot ? mdBlock(state.future_plot) : '—'}</div></div></div></div>`;
}